...
public class TransitionController {
    ...
    static boolean transition(PlayerHolder am, final CompetitionState transition, ArenaTeam team, boolean onlyInMatch,
        ...
        // There is a lot going on in this method, so just to be on the safe side, we're going to catch
        try {
            ...
            Log.err("Unable to perform transition for " + am.getParams().getName() + "!");
    ...
    private static boolean transition(final PlayerHolder am, final CompetitionState transition,
        ...
        try {
            ...
            if (playerReady && !dead) {
                ...
                if (storeAll || mo.hasOption(TransitionOption.STOREEXPERIENCE)) {
                    psc.storeExperience(player);
                }
                if (storeAll || mo.hasOption(TransitionOption.STOREITEMS)) {
                    psc.storeItems(player);
                }
                if (storeAll || mo.hasOption(TransitionOption.STOREHEALTH)) {
                    psc.storeHealth(player);
                }
                ...
                if (storeAll || mo.hasOption(TransitionOption.STOREFLIGHT)) {
                    psc.storeFlight(player);
                }
                if (storeAll || mo.hasOption(TransitionOption.STOREENCHANTS)) {
                    psc.storeEffects(player);
                }
                if (wipeInventory) {
                    InventoryUtil.clearInventory(p);
                }
                if (mo.hasOption(TransitionOption.CLEAREXPERIENCE)) {
                    ExpUtil.setTotalExperience(p, 0);
                }
                if (mo.hasOption(TransitionOption.HEALTH)) {
                    PlayerUtil.setHealth(p, mo.getHealth());
                }
                ...
                if (mo.hasOption(TransitionOption.INVULNERABLE)) {
                    PlayerUtil.setInvulnerable(p, mo.getInvulnerable() * 20);
                }
                if (mo.hasOption(TransitionOption.GAMEMODE)) {
                    PlayerUtil.setGameMode(p, mo.getGameMode());
                }
                if (mo.hasOption(TransitionOption.FLIGHTOFF)) {
                    PlayerUtil.setFlight(p, false);
                }
                if (mo.hasOption(TransitionOption.FLIGHTON)) {
                    PlayerUtil.setFlight(p, true);
                }
                if (mo.hasOption(TransitionOption.FLIGHTSPEED)) {
                    PlayerUtil.setFlightSpeed(p, mo.getFlightSpeed());
                }
                if (mo.hasOption(TransitionOption.DOCOMMANDS)) {
                    PlayerUtil.doCommands(p, mo.getDoCommands());
                }
                if (mo.deEnchant()) {
                    psc.deEnchant(p);
                }
                ...
                if (mo.getExperience() != null) {
                    ExpUtil.giveExperience(p, mo.getExperience());
                }
                if (mo.hasOption(TransitionOption.REMOVEPERMS)) {
                    removePerms(player, mo.getRemovePerms());
                }
                if (mo.hasOption(TransitionOption.ADDPERMS)) {
                    addPerms(player, mo.getAddPerms(), 0);
                }
            ...
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREGAMEMODE)) {
                psc.restoreGamemode(player);
            }
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREEXPERIENCE)) {
                psc.restoreExperience(player);
            }
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREITEMS)) {
                ...
                if (Defaults.DEBUG_TRANSITIONS)
                    Log.info("   " + transition + " transition restoring items " + insideArena);
                psc.restoreItems(player);
            }
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREENCHANTS)) {
                psc.restoreEffects(player);
            }
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREHEALTH)) {
                psc.restoreHealth(player);
            }
            ...
            if (restoreAll || mo.hasOption(TransitionOption.RESTOREFLIGHT)) {
                psc.restoreFlight(player);
            }
            return true;
        } catch (Throwable e){
            Log.err("Unable to perform transition for " + am.getParams().getName() + "!");
